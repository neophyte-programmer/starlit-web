...
import { UPDATE_PROJECT_DETAILS } from "@/utils/server/project"
...
export default function EditProjectForm({ data }: { data: ProjectSchema }) {
    ...
    const [previewImage, setPreviewImage] = useState<any>(data.thumbnail)
    ...
    const editProject = useMutation({
        mutationFn: (values: z.infer<typeof formSchema> & { thumbnail: string}) => {
            ...
            console.log(values)
            return UPDATE_PROJECT_DETAILS(data._id, values, user.token)
        },
        onSuccess: (edited) => {
