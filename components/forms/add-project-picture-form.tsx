...
import { ADD_PROJECT_PICTURE } from "@/utils/server/project"
import toast from "react-hot-toast"
import { Input } from "../ui/input"
import { useRouter } from "next/navigation"

export default function AddProjectPictureForm({ data }: { data: ProjectSchema }) {
    ...
    const modifyMedia = useMutation({
        mutationFn: (values: Media) => {
            ...
            console.log(values)
            return ADD_PROJECT_PICTURE(data._id, values, user.token)
        },
        onSuccess: (edited) => {
    ...
    return (
        <form >
            <div className="w-full max-w-sm flex  flex-col gap-2">
                            <Label>Click to add new image</Label>
                            <Input disabled={fileLoading} onChange={onSubmit} id="thumbnail" accept="image/*" type="file" />
                        </div>
        </form>
    )
}